...
SKIP_CUDA_BUILD = os.getenv(
    "FLASH_ATTENTION_SKIP_CUDA_BUILD", "FALSE") == "TRUE"
# For CI, we want the option to build with C++11 ABI since the nvcr images use C++11 ABI
FORCE_CXX11_ABI = os.getenv(
    "FLASH_ATTENTION_FORCE_CXX11_ABI", "FALSE") == "TRUE"
USE_TRITON_ROCM = os.getenv(
    "FLASH_ATTENTION_TRITON_AMD_ENABLE", "FALSE") == "TRUE"
SKIP_CK_BUILD = os.getenv("FLASH_ATTENTION_SKIP_CK_BUILD",
                          "TRUE") == "TRUE" if USE_TRITON_ROCM else False


@functools.lru_cache(maxsize=None)
def cuda_archs() -> str:
    return os.getenv("FLASH_ATTN_CUDA_ARCHS", "89").split(";")
...
if os.path.isdir(".git"):
    if not SKIP_CK_BUILD:
        subprocess.run(["git", "submodule", "update", "--init",
                       "csrc/composable_kernel"], check=True)
        subprocess.run(["git", "submodule", "update",
                       "--init", "csrc/cutlass"], check=True)
else:
    assert (
...
if not SKIP_CUDA_BUILD:
    ...
    check_if_cuda_home_none("my_flash_attn")
    ...
    compiler_c17_flag = ["-O3", "-std=c++20", "-g"]
    ...
    ext_modules.append(
        CUDAExtension(
            ...
            sources=[
                ...
                "csrc/my_flash_attn/src/flash_fwd_hdim128_fp16_sm80.cu",
                "csrc/my_flash_attn/src/flash_fwd_hdim128_bf16_sm80.cu",
            ...
            include_dirs=[
                ...
                Path(this_dir) / "csrc" / "my_flash_attn" / "src",
...
def get_package_version():
    with open(Path(this_dir) / "my_flash_attn" / "__init__.py", "r") as f:
...
def get_wheel_url():
    ...
    torch_cuda_version = parse(
        "11.8") if torch_cuda_version.major == 11 else parse("12.3")
