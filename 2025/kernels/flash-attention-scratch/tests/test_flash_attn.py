...
def attn_bias_from_alibi_slopes(
    ...
    if causal:
        ...
        if key_leftpad is not None:
            ...
            col_idx = torch.where(col_idx >= key_leftpad,
                                  col_idx - key_leftpad, 2**32)
        sk = (
            seqlen_k
...
def normalize_flash_attn_S(
    ...
    block_size_n = _get_block_size_n(
...
def test_flash_attn_output(
    ...
    if dropout_p > 0.0:
        ...
        if kvpacked:
            kv_rep = repeat(kv, "b s two h d -> b s two (h g) d",
                            g=nheads // nheads_k)
    ...
    if dropout_p > 0.0:
        assert (attn - attn_ref).abs().max().item() <= 2 * \
            (attn_pt - attn_ref).abs().max().item()
        # With alibi, many of the prob values are 0.0 & -0.0 so dropout_fraction isn't accurate
        if not alibi:
            assert abs(dropout_fraction -
                       dropout_p) <= (0.01 if not local else 0.025)
